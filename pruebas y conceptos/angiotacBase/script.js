import { reportStructure, sampleCases } from './data.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const generateConclusion = () => {
        ...
        if (arteriasData && arteriasData.items) {
            ...
            arteriasData.items.forEach(item => {
                if (item.placas) {
                    item.placas.forEach(plaque => {
                        if (plaque.estenosis_severidad && (plaque.estenosis_severidad.includes('Moderna') || placa.estenosis_severidad.includes('Severa') || placa.estenosis_severidad.includes('Oclusión'))) {
    ...
    document.getElementById('save-informe-btn').addEventListener('click', () => {
    ...
    saveBtn.textContent = '¡Guardado!';
    saveBtn.classList.remove('bg-red-600', 'hover:bg-green-700');
    saveBtn.classList.add('bg-green-600');
    
    // Redirigir a informe.html después de un breve delay
    setTimeout(() => {
